<?php

use PHPUnit\Framework\TestCase;
use Mockery as m;
use Doku\Snap\Snap;
use Doku\Snap\Models\Refund\RefundRequestDto;
use Doku\Snap\Models\Refund\RefundResponseDto;
use Doku\Snap\Models\Refund\RefundAdditionalInfoRequestDto;
use Doku\Snap\Models\TotalAmount\TotalAmount;

class DDRefundTest extends TestCase
{
    private $snap;
    private $mockTokenController;
    private $mockDirectDebitController;
    private string $privateKey = "-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----";
private $issuer = "doku";
private $clientId = "BRN-0208-1720408264694";
private $publicKey = "-----BEGIN PUBLIC KEY-----

0QIDAQAB
-----END PUBLIC KEY-----";
    private $dokuPublicKey = "-----BEGIN PUBLIC KEY-----
    
    -----END PUBLIC KEY-----";
    private $secretKey = 'SK-VknOxwR4xZSEPnG7fpJo';
    private $isProduction = "false";
    private $additionalInfo;
    private $totalAmount;

    protected function setUp(): void
    {
        $this->mockTokenController = m::mock(TokenController::class);
        $this->mockDirectDebitController = m::mock(DirectDebitController::class);

        $this->snap = new Snap($this->privateKey, $this->publicKey, $this->dokuPublicKey, $this->clientId, $this->issuer, $this->isProduction, $this->secretKey);
        $this->additionalInfo = new RefundAdditionalInfoRequestDto(
            'DIRECT_DEBIT_CIMB_SNAP'
        );
        $this->totalAmount = new TotalAmount("10000","IDR");
    }

    protected function tearDown(): void
    {
        m::close();
    }

    // Tests for Account Binding - Validate Request
    public function testShouldValidateSuccessfullyWithValidData()
    {
        
        $validData = new RefundRequestDto($this->additionalInfo,"siap300123444438134523234433343335233534432313342234234242837426","213234234",$this->totalAmount,"test","siap300123444438134523234433343335233534432313342234234242837426");
        $this->assertNull($validData->validateRefundRequestDto());
    }
    public function testShouldThrowErrorWhenOriginalPartnerReferenceNoIsMissing()
    {
        $this->expectExceptionMessage("originalPartnerReferenceNo is required");

        $validData = new RefundRequestDto($this->additionalInfo,"","213234234",$this->totalAmount,"test","siap300123444438134523234433343335233534432313342234234242837426");
        $validData->validateRefundRequestDto();
    }

    public function testShouldSuccessfullyRefund()
    {
        $this->mockTokenController->shouldReceive('isTokenInvalid')
            ->with('validToken', 3600, m::type('int'))
            ->andReturn(false);

        $this->mockDirectDebitController->shouldReceive('doRefund')
            ->andReturn(new RefundRequestDto($this->additionalInfo,"siap300123444438134523234433343335233534432313342234234242837426","213234234",$this->totalAmount,"test","siap300123444438134523234433343335233534432313342234234242837426"));
           
        $refundRequestDto = new RefundRequestDto($this->additionalInfo,"siap300123444438134523234433343335233534432313342234234242837426","213234234",$this->totalAmount,"test","siap300123444438134523234433343335233534432313342234234242837426");


         $responseData = $this->snap->doRefund($refundRequestDto,"","","");
         $this->assertNotEmpty($responseData->responseCode);
    }
    


}

