<?php

use PHPUnit\Framework\TestCase;
use Mockery as m;
use Doku\Snap\Snap;
use Doku\Snap\Models\AccountBinding\AccountBindingRequestDto;
use Doku\Snap\Models\AccountBinding\AccountBindingResponseDto;
use Doku\Snap\Models\AccountBinding\AccountBindingAdditionalInfoRequestDto;
use Doku\Snap\Models\AccountBinding\AccountBindingAdditionalInfoResponseDto;

class AccountBindingTest extends TestCase
{
    private $snap;
    private $mockTokenController;
    private $mockDirectDebitController;
    private string $privateKey = "-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----";
private $issuer = "doku";
private $clientId = "BRN-0208-1720408264694";
private $publicKey = "-----BEGIN PUBLIC KEY-----

0QIDAQAB
-----END PUBLIC KEY-----";
    private $dokuPublicKey = "-----BEGIN PUBLIC KEY-----
    
    -----END PUBLIC KEY-----";
    private $secretKey = 'SK-VknOxwR4xZSEPnG7fpJo';
    private $isProduction = "false";
    private $additionalInfo;

    protected function setUp(): void
    {
        $this->mockTokenController = m::mock(TokenController::class);
        $this->mockDirectDebitController = m::mock(DirectDebitController::class);

        $this->snap = new Snap($this->privateKey, $this->publicKey, $this->dokuPublicKey, $this->clientId, $this->issuer, $this->isProduction, $this->secretKey);
        $this->additionalInfo = new AccountBindingAdditionalInfoRequestDto(
            'DIRECT_DEBIT_CIMB_SNAP',
            '12345',
            'John Doe',
            'john.doe@example.com',
            '1234567890',
            'Indonesia',
            'Jl. Merdeka No. 1',
            '19900101',
            'https://example.com/success',
            'https://example.com/failed',
            'iPhone 12',
            'iOS',
            'app'
        );
    }

    protected function tearDown(): void
    {
        m::close();
    }

    // Tests for Account Binding - Validate Request
    public function testShouldValidateSuccessfullyWithValidData()
    {
        
        $validData = new AccountBindingRequestDto('081234567890',  $this->additionalInfo);
        $this->assertNull($validData->validateAccountBindingRequestDto());
    }

    public function testShouldThrowErrorWhenPhoneNoIsMissing()
    {
        $this->expectExceptionMessage("Phone number is required. Example: '62813941306101'.");

        $validData = new AccountBindingRequestDto('',  $this->additionalInfo);

        $validData->validateAccountBindingRequestDto();
    }

    public function testShouldThrowErrorWhenPhoneNoIsLessThan9()
    {
        $this->expectExceptionMessage("phoneNo must be at least 9 digits. Ensure that phoneNo is not empty. Example: '62813941306101'.");


        $validData = new AccountBindingRequestDto('123',  $this->additionalInfo);

        $validData->validateAccountBindingRequestDto();
    }

    public function testShouldThrowErrorWhenPhoneNoIsMoreThan16()
    {
        $this->expectExceptionMessage("phoneNo must be 16 characters or fewer. Ensure that phoneNo is no longer than 16 characters. Example: '62813941306101'.");
        $validData = new AccountBindingRequestDto('081234567890123456',  $this->additionalInfo);
        $validData->validateAccountBindingRequestDto();
    }

    public function testShouldThrowErrorWhenChannelIsMissing()
    {
        $this->expectExceptionMessage("additionalInfo.channel cannot be null. Ensure that additionalInfo.channel is one of the valid channels. Example: 'DIRECT_DEBIT_ALLO_SNAP'.");
        $this->additionalInfo->channel = '';
        $validData = new AccountBindingRequestDto('08129898787',  $this->additionalInfo);

        $validData->validateAccountBindingRequestDto();
    }

    // Tests for Account Binding - Process
    public function testShouldSuccessfullyBindAccount()
    {
        $additionalInfo = new AccountBindingAdditionalInfoResponseDto("cust1010","status","authCode");
        $this->mockTokenController->shouldReceive('isTokenInvalid')
            ->with('validToken', 3600, m::type('int'))
            ->andReturn(false);

        $this->mockDirectDebitController->shouldReceive('doAccountBinding')
            ->andReturn(new AccountBindingResponseDto(
                '200', 'Success', 'REF123', 'https://example.com/redirect',  $additionalInfo
            ));
            $additionalInfo = new AccountBindingAdditionalInfoRequestDto(
                'DIRECT_DEBIT_CIMB_SNAP',
                '12345',
                'John Doe',
                'john.doe@example.com',
                '1234567890',
                'Indonesia',
                'Jl. Merdeka No. 1',
                '19900101',
                'https://example.com/success',
                'https://example.com/failed',
                'iPhone 12',
                'iOS',
                'app'
            );
        $accountBindingRequestDto = new AccountBindingRequestDto('081234567890', $additionalInfo);

        $response = $this->snap->doAccountBinding(
            $accountBindingRequestDto,
            '192.168.0.1',
            'deviceId'
        );
         $this->assertNotEmpty($response->responseCode);
    }
}

