<?php

use PHPUnit\Framework\TestCase;
use Mockery as m;
use Doku\Snap\Snap;
use Doku\Snap\Models\AccountUnbinding\AccountUnbindingRequestDto;
use Doku\Snap\Models\AccountUnbinding\AccountUnbindingResponseDto;
use Doku\Snap\Models\AccountUnbinding\AccountUnbindingAdditionalInfoRequestDto;

class DDCardUnregistrationTest extends TestCase
{
    private $snap;
    private $mockTokenController;
    private $mockDirectDebitController;
    private string $privateKey = "-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----";
private $issuer = "doku";
private $clientId = "BRN-0208-1720408264694";
private $publicKey = "-----BEGIN PUBLIC KEY-----

0QIDAQAB
-----END PUBLIC KEY-----";
    private $dokuPublicKey = "-----BEGIN PUBLIC KEY-----
    
    -----END PUBLIC KEY-----";
    private $secretKey = 'SK-VknOxwR4xZSEPnG7fpJo';
    private $isProduction = "false";
    private $additionalInfo;

    protected function setUp(): void
    {
        $this->mockTokenController = m::mock(TokenController::class);
        $this->mockDirectDebitController = m::mock(DirectDebitController::class);

        $this->snap = new Snap($this->privateKey, $this->publicKey, $this->dokuPublicKey, $this->clientId, $this->issuer, $this->isProduction, $this->secretKey);
        $this->additionalInfo = new AccountUnbindingAdditionalInfoRequestDto(
            'DIRECT_DEBIT_CIMB_SNAP'
        );
    }

    protected function tearDown(): void
    {
        m::close();
    }

    // Tests for Account Binding - Validate Request
    public function testShouldValidateSuccessfullyWithValidData()
    {
        
        $validData = new AccountUnbindingRequestDto('token',  $this->additionalInfo);
        $this->assertNull($validData->validateAccountUnbindingRequestDto());
    }

    public function testShouldThrowErrorWhenTokenIsMissing()
    {
        $this->expectExceptionMessage("Token ID is required");

        $validData = new AccountUnbindingRequestDto('',  $this->additionalInfo);

        $validData->validateAccountUnbindingRequestDto();
    }

    public function testShouldThrowErrorWhenChannelIsMissing()
    {
        $this->expectExceptionMessage("additionalInfo.channel cannot be null or empty. Ensure that additionalInfo.channel is one of the valid channels. Example: 'DIRECT_DEBIT_ALLO_SNAP'.");
        $this->additionalInfo->channel = '';
        $validData = new AccountUnbindingRequestDto('08129898787',  $this->additionalInfo);

        $validData->validateAccountUnbindingRequestDto();
    }

    // Tests for Account Binding - Process
    public function testShouldSuccessfullyUnbindAccount()
    {
        $this->mockTokenController->shouldReceive('isTokenInvalid')
            ->with('validToken', 3600, m::type('int'))
            ->andReturn(false);

        $this->mockDirectDebitController->shouldReceive('doAccountUnbinding')
            ->andReturn(new AccountUnbindingResponseDto(
                '200', 'Success', 'REF123', 'https://example.com/redirect',  $this->additionalInfo
            ));
           
        $AccountUnbindingRequestDto = new AccountUnbindingRequestDto('token', $this->additionalInfo);

        $response = $this->snap->doCardUnbinding(
            $AccountUnbindingRequestDto
        );

        $this->assertNotEmpty($response->responseCode);
    }
}

